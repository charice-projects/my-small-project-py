"""
core/content_formatter.py - 修复版本
...
class ContentFormatter:
    ...
    def format_content(self, content: str, content_type: str = 'ai') -> str:
        ...
        if not isinstance(content, str):
            self.logger.warning(f"content不是字符串，类型: {type(content)}，尝试转换")
            try:
                content = str(content)
            except Exception as e:
                self.logger.error(f"无法将content转换为字符串: {e}")
                return ""
        ...
        try:
            ...
            # 步骤4: 恢复代码块
            formatted_content = self._restore_code_blocks(protected_content, code_blocks)
            ...
            self.logger.error(f"格式化内容失败: {e}")
            # 如果格式化失败，返回原始内容
            return content
    
    def _protect_code_blocks(self, content: str) -> Tuple[str, Dict[str, Dict]]:
        """保护代码块，防止后续处理破坏它们"""
        # 查找所有代码块
        code_blocks = {}
        ...
        # 确保content是字符串
        if not isinstance(content, str):
            self.logger.warning(f"_protect_code_blocks: content不是字符串，类型: {type(content)}")
            try:
                content = str(content)
            except Exception as e:
                self.logger.error(f"无法将content转换为字符串: {e}")
        ...
        try:
            ...
            self.logger.error(f"正则表达式替换失败: {e}")
    ...
    def _clean_html_tags(self, content: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"清理HTML标签失败: {e}")
        
        return content
    
    def _restore_code_blocks(self, protected_content: str, code_blocks: Dict[str, Dict]) -> str:
        """恢复代码块"""
        if not protected_content or not isinstance(protected_content, str):
            return protected_content or ""
    ...
    def _adjust_heading_levels(self, content: str, base_level: int = 2) -> str:
        ...
        try:
            ...
            self.logger.error(f"调整标题层级失败: {e}")
            return content
    ...
    def _remove_emojis(self, content: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"移除表情符号失败: {e}")
            return content
    ...
    def _normalize_whitespace(self, content: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"规范化空白字符失败: {e}")
            return content
...
{code}
                ...
                # 普通代码块 
                formatted = f'''{header} {code} ```'''
            
            return formatted
        except Exception as e:
            self.logger.error(f"格式化代码块失败: {e}")
